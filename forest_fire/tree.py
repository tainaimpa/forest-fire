...
class Tree(mesa.Agent):
    def __init__(self, unique_id, model, pos, size: float, color: str, tree_density, reprod_speed=1):
    ...
    def grow_tree(self, growable_agent):
        ...
        color = self.model.biome.color
        tree = Tree(self.model.next_id(), self.model, pos, size, color, self.model.tree_density, self.model.reprod_speed)
